...
def contains_framework(framework, item):
    ...
    import_regex = (
        r"(from.*" + re.escape(framework) + r"|import.*" + re.escape(framework) + r")"
    )
        ...
        for i in range(len(ast)):
            ...
            if word.type == "identifier" and word.text in set:
                if (
                    i != len(ast) - 1
                    and ast[i + 1].text == "("
                    and ((i != 0 and ast[i - 1].text != "def") or i == 0)
                ):
...
def main():
    ds = load_dataset("codeparrot/codeparrot-clean", streaming=False, split="train")
    ...
    if test_negatives:
        ds = ds.filter(
            partial(
                lambda framework, x: not contains_framework(framework, x), "tensorflow"
            )
        )
        ds = ds.filter(
    ...
    ds = ds.map(
        partial(
            get_name_frequencies, starting_dict, function_list, framework, debug=False
        ),
        batched=False,
        remove_columns=[
            "alpha_frac",
