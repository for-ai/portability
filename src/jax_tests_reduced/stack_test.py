    ...
    def testStack(self, shape, axis, dtypes, array_input, out_dtype):
        ...
        if numpy_version < (1, 24):
            np_fun = jtu.promote_like_jnp(
                lambda *args: np.stack(*args, axis=axis).astype(out_dtype)
            )
        else:
            np_fun = jtu.promote_like_jnp(
                partial(np.stack, axis=axis, dtype=out_dtype, casting="unsafe")
            )

        jnp_fun = partial_timed(jnp.stack, axis=axis, dtype=out_dtype)
