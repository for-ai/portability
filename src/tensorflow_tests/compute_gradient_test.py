...
class MiniMNISTTest(test.TestCase):
    ...
    def _BuildAndTestMiniMNIST(self, param_index, tag):
        ...
        label_data = np.random.random(
            batch * classes).reshape((batch, classes))
        ...
        all_params = [inp, hidden_weight,
                      hidden_bias, softmax_weight, softmax_bias]
