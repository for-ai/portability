...
class StudentTTest(test.TestCase):
    ...
    def testStudentPDFAndLogPDF(self):
        ...
        student = student_t.StudentT(
            df, loc=mu, scale=-sigma)  # pylint: disable=invalid-unary-operand-type
    ...
    def testStudentCDFAndLogCDF(self):
        ...
        self.assertAllClose(
            expected_log_cdf, log_cdf_values, atol=0., rtol=1e-5)
        self.assertAllClose(
    ...
    def testStudentSampleMultiDimensional(self):
        ...
        self.assertAllClose(
            ...
            atol=0)
        self._checkKLApprox(
            df_v[0], mu_v[0], sigma_v[0], sample_values[:, 0, 0])
        self.assertAllClose(
        ...
        self.assertAllClose(
            ...
            atol=0)
        self._checkKLApprox(
            df_v[1], mu_v[1], sigma_v[1], sample_values[:, 0, 1])
    ...
    def testBroadcastingPdfArgs(self):
        ...
        _check2d_rows(student_t.StudentT(
            df=[[2.], [3.], [4.]], loc=2., scale=1.))
        _check2d_rows(student_t.StudentT(
            df=7., loc=[[2.], [3.], [4.]], scale=1.))
        _check2d_rows(student_t.StudentT(
            df=7., loc=3., scale=[[2.], [3.], [4.]]))

    def testMeanAllowNanStatsIsFalseWorksWhenAllBatchMembersAreDefined(self):
        mu = [1., 3.3, 4.4]
        student = student_t.StudentT(
            df=[3., 5., 7.], loc=mu, scale=[3., 2., 1.])
