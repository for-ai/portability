...
class VariablesToConstantsTest(test.TestCase):
    ...
    def testIf(self):
        ...
        weights = variables.Variable(
            [[0.1, 0.2], [0.3, 0.4]], dtype=dtypes.float32)
    ...
    def testWhile(self):
        """Test a While loop."""
        input_data = {"x": constant_op.constant(
            [1., 2., 3., 4.], shape=[2, 2])}
    ...
    def testSwitchCase(self):
        ...
        w2 = variables.Variable(
            np.random.random_sample((4,)), dtype=np.float32)
    ...
    def _testConvertedFunction(self, sess, obj, func, converted_concrete_func,
        ...
        for expected, actual in zip(expected_value, actual_value):
            np.testing.assert_almost_equal(
                sess.run(expected), sess.run(actual))
    ...
    def testConvertVariables(self):
        ...
        with ops.Graph().as_default():
            with session_lib.Session() as sess:
                ...
                self._testConvertedFunction(
                    sess, root, root.f, output_func, input_data)
    ...
    def testVariableSavedModel(self):
        ...
        with ops.Graph().as_default():
            with session_lib.Session() as sess:
                ...
                variable_graph_def = input_func.graph.as_graph_def()
                self.assertTrue(
                    has_stateful_partitioned_call_op(variable_graph_def))
            ...
            with session_lib.Session() as sess:
                ...
                @def_function.function(input_signature=[
                    tensor_spec.TensorSpec(
                        shape=[3, 10, 10], dtype=dtypes.float32)
                ])
                def model(x):
                    return rnn.dynamic_rnn(cell, x, dtype=dtypes.float32)
        ...
        with ops.Graph().as_default():
            with session_lib.Session() as sess:
                input_data = {
                    "i":
                        constant_op.constant(
                            np.random.randint(0, 3, dtype=np.int32)),
                    "x":
                        constant_op.constant(
                            np.asarray(
                ...
                @def_function.function(input_signature=[
                    tensor_spec.TensorSpec(shape=[], dtype=dtypes.int32),
                    tensor_spec.TensorSpec(
                        shape=[10, 3], dtype=dtypes.float32),
                ])
                def model(i, x):
    ...
    def test_resource_variable_can_be_written_after_denylisting(self):
        with ops.Graph().as_default():
            with variable_scope.variable_scope("", use_resource=True):
                ...
                with ops.control_dependencies(
                    ...
                    output_node = array_ops.identity(
                        variable_node, name="output_node")
        ...
        with ops.Graph().as_default():
            _ = importer.import_graph_def(
                constant_graph_def_with_denylist, name="")
            with session_lib.Session() as sess:
                output_node = sess.graph.get_tensor_by_name("output_node:0")
                self.evaluate(
                    sess.graph.get_operation_by_name(initializer_name))
                output = self.evaluate(output_node)
                self.assertNear(3.0, output, 0.00001)
                output = self.evaluate(output_node)
    ...
    def testGraphWithSwitch(self):
        ...
        with ops.Graph().as_default():
            with variable_scope.variable_scope("", use_resource=True):
                ...
                def f1(): return variable_scope.get_variable("var_f1", initializer=17.0)
                def f2(): return variable_scope.get_variable("var_f2", initializer=23.0)
    ...
    def _addNoinlineAttributeToFunction(self, saved_model_dir, func_name):
        ...
        for func_def in new_meta_graph_def.graph_def.library.function:
            ...
            if func_name_without_prefix.startswith(func_name):
                func_def.attr["_noinline"].CopyFrom(
    ...
    def testVariableModelWithFunctionAndFunctionInliningDisabled(self):
        ...
        with ops.Graph().as_default():
            ...
            with session_lib.Session() as sess:
                sess.run(variables.global_variables_initializer())
                simple_save.simple_save(
                    sess, save_dir, {"myinput": a}, {"myoutput": b})
