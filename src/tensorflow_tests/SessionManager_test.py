...
class SessionManagerTest(test.TestCase):
    ...
    def testPrepareSessionFails(self):
        ...
        with ops.Graph().as_default():
            ...
            with self.cached_session():
                self.assertEqual(
                    False, variables.is_variable_initialized(v).eval())
        ...
        with ops.Graph().as_default():
            ...
            with self.cached_session():
                self.assertEqual(
                    False, variables.is_variable_initialized(v).eval())
                self.assertEqual(
                    False, variables.is_variable_initialized(w).eval())
            sm2 = session_manager.SessionManager(
                ready_op=variables.report_uninitialized_variables(),
                ready_for_local_init_op=variables.report_uninitialized_variables(
                    variables.global_variables()),
        ...
        with ops.Graph().as_default():
            ...
            with self.cached_session():
                ...
                self.assertEqual(
                    False, variables.is_variable_initialized(w).eval())
                self.assertEqual(
                    False, variables.is_variable_initialized(x).eval())
            sm2 = session_manager.SessionManager(
                ready_op=variables.report_uninitialized_variables(),
                ready_for_local_init_op=variables.report_uninitialized_variables(
                    variables.global_variables()),
        ...
        with ops.Graph().as_default():
            ...
            with self.cached_session():
                ...
                self.assertEqual(
                    False, variables.is_variable_initialized(x).eval())
                self.assertEqual(
                    False, variables.is_variable_initialized(v_res).eval())
                self.assertEqual(
                    False, variables.is_variable_initialized(w_res).eval())
                self.assertEqual(
                    False, variables.is_variable_initialized(x_res).eval())
    ...
    def testPrepareSessionWithCyclicInitializer(self):
        ...
        # cyclic dependencies.
        with ops.Graph().as_default():
            i = control_flow_ops.while_loop(
                lambda i: i < 1, lambda i: i + 1, [0])
