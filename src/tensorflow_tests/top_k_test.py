...
class TopKTest(test.TestCase):
            ...
            if sorted:
                ...
                if not np.issubdtype(np_expected_values.dtype, np.floating):
                    ...
                    if np.any(indices_not_equal):
                        ...
                        self.assertAllClose(
                            expected_values_unsure, values_unsure)
            else:
                ...
                self.assertAllEqual(
                    np.sort(np_expected_indices), np.sort(indices))
                self.assertAllClose(
                    np.sort(np_expected_values), np.sort(values))
    ...
    def testTop2(self):
        ...
        self._validateTopK(
            inputs, 2, [[0.4, 0.3], [0.4, 0.3]], [[3, 1], [2, 1]])
            ...
            with ops.Graph().as_default():
                ...
                with session.Session() as sess:
                    ...
                    r = self.run_op_benchmark(
                        sess, op, min_iters=100, name=name)
