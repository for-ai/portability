...
class TestTensorCreation(TestCase):
    ...
    def test_cat_all_dtypes_and_devices(self, device):
        for dt in all_types_and_complex_and(torch.half, torch.bool, torch.bfloat16, torch.chalf):
            ...
            expected2 = torch.tensor(
                [[1, 2, 1, 2], [3, 4, 3, 4]], dtype=dt, device=device)
            with pytorch_op_timer():
                test_2 = torch.cat((x, x), 1)
            self.assertEqual(test_2, expected2)
    ...
    def test_cat_in_channels_last(self, device):
        for dim in range(4):
            ...
            self.assertTrue(res2.is_contiguous(
    ...
    def test_cat_preserve_channels_last(self, device):
        ...
        with pytorch_op_timer():
            res1 = torch.cat((x, y))
        with pytorch_op_timer():
            res2 = torch.cat((x.contiguous(memory_format=torch.channels_last), y.contiguous(
            memory_format=torch.channels_last)))
        self.assertEqual(res1, res2)
        self.assertTrue(res2.is_contiguous(memory_format=torch.channels_last))
        # discontiguous channels-last inputs
        x = torch.arange(24, dtype=torch.float, device=device).reshape(
            2, 2, 3, 2).to(memory_format=torch.channels_last)
        x1 = x[:, :, :2]
        x2 = x[:, :, 1:]
        with pytorch_op_timer():
    ...
    def test_cat_out_memory_format(self, device):
        ...
        out_cpu = torch.empty(expected_size, device='cpu').contiguous(
            memory_format=torch.contiguous_format)
        with pytorch_op_timer():
            res1_cpu = torch.cat((a_cpu, b_cpu), out=out_cpu)

        self.assertTrue(res1_cuda.is_contiguous(
            memory_format=torch.contiguous_format))
        self.assertTrue(res1_cpu.is_contiguous(
            memory_format=torch.contiguous_format))
        ...
        out_cuda = torch.empty((0), device=device).contiguous(
            memory_format=torch.contiguous_format)
        # a_cuda and b_cuda have different memory_format
        with pytorch_op_timer():
            res2_cuda = torch.cat((a_cuda, b_cuda), out=out_cuda)

        out_cpu = torch.empty((0), device='cpu').contiguous(
            memory_format=torch.contiguous_format)
        with pytorch_op_timer():
            res2_cpu = torch.cat((a_cpu, b_cpu), out=out_cpu)

        self.assertTrue(res2_cuda.is_contiguous(
            memory_format=torch.contiguous_format))
        self.assertTrue(res2_cpu.is_contiguous(
            memory_format=torch.contiguous_format))
        ...
        self.assertTrue(res3_cuda.is_contiguous(
            memory_format=torch.channels_last))
    ...
    def test_cat_big(self, device):
        ...
        SIZE2 = 4500
        concat_list = []
        concat_list.append(torch.ones((SIZE1, 1024 * 512),
                           dtype=torch.uint8, device=device))
        concat_list.append(torch.ones((SIZE2, 1024 * 512),
                           dtype=torch.uint8, device=device))
        with pytorch_op_timer():
    ...
    def test_cat2(self, device, dtype):
        ...
        for dim in range(-3, 3):
            pos_dim = dim if dim >= 0 else 3 + dim
            x = torch.randint(low=-100, high=100, size=(13, SIZE, SIZE),
                              device=device).to(dtype).transpose(0, pos_dim)
            y = torch.randint(low=-100, high=100, size=(17, SIZE, SIZE),
                              device=device).to(dtype).transpose(0, pos_dim)
            z = torch.randint(low=-100, high=100, size=(19, SIZE, SIZE),
                              device=device).to(dtype).transpose(0, pos_dim)
            with pytorch_op_timer():
                res1 = torch.cat((x, y, z), dim)
            self.assertEqual(res1.narrow(pos_dim, 0, 13), x, atol=0, rtol=0)
        ...
        x = torch.randint(low=-100, high=100, size=(20, SIZE,
                          SIZE), device=device).to(dtype)
        with pytorch_op_timer():
            test_1 = torch.cat(torch.split(x, 7))
        self.assertEqual(test_1, x)
        with pytorch_op_timer():
            test_2 = torch.cat(torch.chunk(x, 7))
        self.assertEqual(test_2, x)

        y = torch.randint(low=-100, high=100, size=(1, SIZE,
                          SIZE), device=device).to(dtype)
        with pytorch_op_timer():
            z = torch.cat([x, y])
        self.assertEqual(z.size(), (21, SIZE, SIZE))
