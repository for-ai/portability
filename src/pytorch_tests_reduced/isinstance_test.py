...
class TestJit(JitTestCase):
    def test_isinstance(self, device):
        ...
        inputs = [True, 1, 1.0, torch.tensor(1, device=device), [1, 2], (1.0,), [1, 2], 1]
    ...
    def test_isinstance_metacompile(self, device):
        @torch.jit.script
        ...
        out = test_non_primitive_types(_MyNamedTuple(value=torch.tensor(5.0, device=device)))
        self.assertEqual(out, torch.tensor(6.0, device=device))


    def test_isinstance_dynamic(self, device):
        @torch.jit.script
        def foo(a):
    ...
    def test_isinstance_refinement(self, device):
        @torch.jit.script
        def foo(a):
        ...
        self.assertEqual(any_refinement2(3), torch.tensor(3, device=device))
        self.assertEqual(any_refinement2(torch.tensor(5, device=device)), torch.tensor(5, device=device))
