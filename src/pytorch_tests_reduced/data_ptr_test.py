...
class TestTorch(TestCase):
    def test_to(self, device):
        ...
        if is_cuda10_2_or_higher:  # in cuda10_1 sparse_csr is beta
            self._test_to_with_layout(torch.sparse_csr, device=device)
    ...
    def _test_to_with_layout(self, layout, device):
        ...
        if layout == torch.sparse_csr:
            a = torch.tensor([[0, 1, 2], [2, 0, 3]], device=device).to_sparse_csr()
        test_copy_behavior(a)
        self.assertEqual(a.device, a.device)
        self.assertEqual(a.device, a.to(dtype=torch.float32).device)
        ...
        def test_data_ptr(getter):
            ...
            with pytorch_op_timer():
                test_2 = getter(a)
            self.assertEqual(test_2, getter(
                a.to(dtype=a.dtype, device=a.device, copy=False)))
            with pytorch_op_timer():
                test_3 = getter(a)
            self.assertEqual(test_3, getter(a.to('cpu', copy=False)))
            with pytorch_op_timer():
                test_4 = getter(a)
            self.assertNotEqual(test_4, getter(a.to('cpu', copy=True)))
        if layout == torch.sparse_csr:
