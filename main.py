...
def get_name_frequencies(i):
    string = "\n".join(i['content'])
    dict = {}
    for word in ctok.tokenize(string, lang="python", syntax_error="ignore"):
...
def main():
    ...
    ds = ds.map(get_name_frequencies, batched=True, batch_size=50, remove_columns=[
                "alpha_frac", "autogenerated", "content", 'copies', 'hash', 'license', 'line_max', 'line_mean', 'path', 'repo_name', 'size'])
